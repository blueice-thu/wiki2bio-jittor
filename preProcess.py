...
def split_infobox():
    ...
    *.box.pos is the position counted from the begining of a field
    ...
    boxes = ["original_data/train.box",
             "original_data/valid.box", "original_data/test.box"]
...
def reverse_pos():
    ...
    bpfile = ["processed_data/train/train.box.pos",
              "processed_data/valid/valid.box.pos", "processed_data/test/test.box.pos"]
    bwfile = ["processed_data/train/train.box.rpos",
              "processed_data/valid/valid.box.rpos", "processed_data/test/test.box.rpos"]
...
def split_summary_for_rouge():
    ...
    bwfile = ["processed_data/test/test_split_for_rouge/",
              "processed_data/valid/valid_split_for_rouge/"]
...
class Vocab(object):
    ...
    def __init__(self):
        ...
        vocab['UNK_TOKEN'] = 3
        cnt = 4
        with open("original_data/word_vocab.txt", "r", encoding='utf-8') as v:
            for line in v:
        ...
        key_map['UNK_TOKEN'] = 3
        cnt = 4
        with open("original_data/field_vocab.txt", "r", encoding='utf-8') as v:
            for line in v:
...
def table2id():
    ...
    for k, ff in enumerate(fvals):
        ...
        for line in fi:
            ...
            fo.write(" ".join([str(vocab.word2id(word))
                     for word in items]) + '\n')
        fi.close()
        fo.close()
    for k, ff in enumerate(flabs):
        ...
        for line in fi:
            items = line.strip().split()
            fo.write(" ".join([str(vocab.key2id(key))
                     for key in items]) + '\n')
        fi.close()
        fo.close()
